...
for service in rsyslog redis-server zebra postsrsd unattended-upgrades clamav-daemon; do
...
for service in bind9 dovecot postfix php7.0-fpm incron cron rbldnsd nginx; do
service $service reload || service $service restart > /dev/null 2>&1 
done
