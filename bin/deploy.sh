...
if ! echo ${debian_version} | grep -q "^9\."; then
	printf "Current version of Scrollout works on Debian 9 (Stretch) only. Aborting...\n";
	exit
else
	debian_dist="stretch"
...
echo "deb http://cdn-fastly.deb.debian.org/debian ${debian_dist}-backports main" > /etc/apt/sources.list.d/${debian_dist}-backports.list
...
sudo apt-get install libdigest-sha-perl -y
sudo apt-get install libdigest-sha3-perl -y
...
sudo apt-get autoremove php5-fpm -y
...
if [ -f /etc/apt/sources.list.d/${debian_dist}-backports.list ]; then
	for package in amavisd-new clamav-daemon nginx-extras mailgraph ntpdate php-fpm postfix cifs-utils tzdata postfix-ldap postfix-pcre rbldnsd postfix-policyd-spf-python postsrsd; do 
		sudo DEBIAN_FRONTEND=noninteractive apt-get install -t ${debian_dist}-backports -qy -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" $package
	done
else
	for package in amavisd-new clamav-daemon nginx-extras mailgraph ntpdate php-fpm postfix cifs-utils tzdata postfix-ldap postfix-pcre rbldnsd postfix-policyd-spf-python postsrsd; do 
		sudo DEBIAN_FRONTEND=noninteractive apt-get install -qy -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" $package
	done
fi
...
echo "ntpdate -u pool.ntp.org time.windows.com ntp.ubuntu.com > /dev/null 2>&1" >> /etc/cron.hourly/ntpdate
...
sed -i "s/.*short_open_tag .*/short_open_tag = On/" /etc/php/*/fpm/php.ini
sed -i "s/.*max_input_vars .*/max_input_vars = 25000/" /etc/php/*/fpm/php.ini
sed -i "s/.*max_execution_time .*/max_execution_time = 300/" /etc/php/*/fpm/php.ini
sed -i "s/.*memory_limit .*/memory_limit = 128M/" /etc/php/*/fpm/php.ini
sed -i "s/.*suhosin.post.max_vars .*/suhosin.post.max_vars = 25000/" /etc/php/*/fpm/php.ini
sed -i "s/.*suhosin.request.max_vars .*/suhosin.request.max_vars = 25000/" /etc/php/*/fpm/php.ini
...
localip=`ip a | awk '!/127\.0\.0/ && /inet /' | /usr/bin/awk '/inet .*scope global/ {print $2; exit;}' | /usr/bin/awk -F '/' '{print $1}'`;
