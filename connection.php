...
$ipaddr = exec("sudo /sbin/ip a s dev ". $interface ." | /usr/bin/awk '!/127\.0\.0/ && /inet /' | /usr/bin/awk '/inet .*scope global/ {print $2; exit;}' | /usr/bin/awk -F '/' '{print $1}'");
$ipaddr6 = exec("sudo /sbin/ip -6 a s dev ". $interface ." | /usr/bin/awk '/inet6 .*scope global/ {print $2; exit;}' | /usr/bin/awk -F '/' '{print $1}'");
$maskaddr = exec("sudo /sbin/ip a s dev ". $interface ." | /usr/bin/awk '!/127\.0\.0/ && /inet /' | /usr/bin/awk '/inet .*scope global/ {print $2; exit;}' | /usr/bin/awk -F '/' '{print $2}'");
